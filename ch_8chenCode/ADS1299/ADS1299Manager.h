...
//Pick which version of 211bci you have
#define a211bci_V1 (1)    //Sept 2013
#define a211bci_V2 (2)    //Oct 24, 2013
#define a211bci_NCHAN (8)  // number of EEG channels
...
#define PIN_RST (9)
#define PIN_CS (10)
#define SCK_MHZ (4)    //FREQ=4£¬DIVIDER = SPI_CLOCK_DIV_4;
...
{
  public:
    void initialize(void);                                     //initialize the ADS1299 controller.  Call once.  Assumes 211bci_V2
    void initialize(int version,boolean isDaisy);                              //initialize the ADS1299 controller.  Call once.  Set which version of 211bci you're using.
    void setVersion211bci(int version);			//Set which version of 211bci you're using.
    ...
    void writeChannelDataAsBinaryToAndroid(int N, long int sampleNumber);
    
  private:
    boolean use_neg_inputs;
    boolean use_SRB2[a211bci_NCHAN];
